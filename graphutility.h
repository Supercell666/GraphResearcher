...
namespace webgr
{
    ...
    template<typename handler_fcn_type>
    // ??? ???????????????? ???????????????? ??????????
    void foreach_ordered(unsigned left, unsigned right, const handler_fcn_type& fcn)
    {
        unsigned prev_d = (right - left);
        unsigned d = prev_d;
        while (d>0)
        {
            for (unsigned i = left+d; i <= right - d; i += d)
            {
                if ((i - left)%prev_d != 0)
                    fcn(i);
            }
        ...
        std::thread::hardware_concurrency())
    {
        ...
        auto t = (end - begin) / step;
        /*if (t < 0)
            t = -t;*/
        if (t > type(1))
        {
            unsigned short size = thread_count - 1;
            std::thread** threads = new std::thread*[size];
            for (unsigned id = 0; id < size; ++id)
            {
                threads[id] = new std::thread([id, &step, &size,
                    &begin, &end, &func]()
                {
                    for (type it = begin + step*id; it < end;
                        it += step*size)
                    {
                        func(it, id);
                    }
            ...
            for (type it = begin + step*size; it < end;
                it += step*size)
            {
                func(it, size);
            }
        ...
        {
            for (type it = begin; it < end; it += step)
            {
                func(it, 0);
            }
        }
        ...
        unsigned max_cl_size, double p_in_cl = 0.5, double p_out_cl = 0.3)
    {
        ...
        for (unsigned b_begin = 0, cl = 0; cl < size.size(); ++cl)
        {
            ...
            auto& ret_cl = ret.clusters[cl];
            for (unsigned i = b_begin; i < b_end; ++i)
            {
                ret_cl.g.push_back(i);
                ret.vertexes[i] = cl;
    ...
    double assort(const std::string& gr)
    {
        ...
        while (getline(fin, str))
        {
            ...
            d_sum += in_it;
            f_sum += out_it;
            d_sqr += (double)in_it * in_it;
            f_sqr += (double)out_it * out_it;
            df_sum += (double)in_it * out_it;
        }
    ...
    // ????????? ????? ?? ?????(*.net, *.gv)
    default_graph from_file(const std::string& path, bool need_name = true)
    {
        int pos = path.rfind('.');
        if (pos == std::string::npos)
            throw std::runtime_error(("Unknown file extension: " + path).c_str());
        std::string file_ext(path.substr(pos + 1));
        if (file_ext == "gv")
            return from_gv(path);
        else if (file_ext == "net")
            return from_pajek(path, need_name);
        else
            throw std::runtime_error(("Unknown file extension: " + path).c_str());
    }
    ...
    std::ostream& operator<<(std::ostream& os, const std::vector<std::set<
        default_graph::vertex_name_type>>&res)
    {
