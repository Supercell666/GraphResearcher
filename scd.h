...
void scd::del_outtriangl_edges()
{
    ...
    {
        std::forward_list<unsigned> t;

        typename std::map<unsigned, decltype(temp_graph)::edge_type>::
                const_iterator ob = i.second->output.upper_bound(i.first),
                oe = i.second->output.end(), ib = i.second->input.upper_bound(i.first),
                ie = i.second->input.end();
        merging(ob, oe, ib, ie, [&t](
                const std::pair<unsigned, unsigned>& p)
        {
            t.push_front(p.first);
        });
        ...
        {
            auto* p=temp_graph[k];
            ob = p->output.upper_bound(k);
            oe = p->output.end();
            ib = p->input.upper_bound(k);
            ie = p->input.end();
            merging(ob, oe, ib, ie, [this, &t, &i, &i_it, &k](const std::pair<unsigned, unsigned>& p)
            {
...
bool scd::clusterizate2()
{
    ...
    do
    {
        ...
        for (auto it = temp_graph.begin(); it != temp_graph.end(); ++it)
        {
            ...
            ib = p->input.begin(),
            ie = p->input.end();
            merging(ob, oe, ib, ie, [this, &moves, &it, &dest, &this_](
                    const std::pair<unsigned, unsigned>& p)
            {
